...
const CartView: React.FC<CartViewProps> = ({
  ...
  if (itemCurrency === "GBP") currencySymbol = "£";
    ...
    const storeRegion = items[0]?.store
      ? getStoreRegion(items[0].store)
      : "USA";
    const currencyConfig =
      STORE_ADDRESSES[storeRegion] || STORE_ADDRESSES["USA"];
  ...
  return (
    <View style={styles.screen}>
      ...
      <Text style={styles.subText}>
        {items.length} {items.length === 1 ? "item" : "items"} staging for
        import
      </Text>
          ...
          <Card style={styles.checkoutSummary}>
            <View style={styles.summaryRow}>
              ...
              <Text style={styles.summaryValue}>
                {currencySymbol}
                {items.reduce((a, b) => a + b.price * b.quantity, 0).toFixed(2)}
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  cartStore: { color: "#1C39BB", fontSize: 12, fontFamily: "Satoshi-Medium" },
  cartTitle: { color: "#000", fontSize: 16, fontFamily: "Satoshi-Medium" },
  cartBottom: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  cartQty: { color: "#000", fontSize: 14, fontFamily: "Satoshi-Medium" },
  cartPrice: { color: "#000", fontSize: 16, fontFamily: "Satoshi-Bold" },
  checkoutSummary: {
    padding: 24,
    paddingBottom: 34,
    marginTop: "auto",
  ...
  summaryValue: { color: "#000", fontSize: 24, fontFamily: "Satoshi-Bold" },
  checkoutBtn: {
    width: "100%",
    shadowColor: "#1C39BB",
    shadowOpacity: 0.3,
